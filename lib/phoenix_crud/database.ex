defmodule PhoenixCrud.Database do
  use GenServer
  require Logger
  ...
  defp try_start() do
    case PhoenixCrud.Repo.start_link([show_sensitive_data_on_connection_error: true]) do
      {:ok, pid} ->
        pid
      {:error, {:already_started, _pid}} ->
        raise "Wasn't expecting RepoCrud.Repo to be started but it was!"
      {:error, reason} ->
        Logger.warn "Unable to start RepoCrud.Repo: #{inspect reason}"
        nil
    end
  end
end
